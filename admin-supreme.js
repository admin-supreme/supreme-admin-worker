...
async function updateAnime(request, db, id, env, ctx) {
  ...
  try {
    ...
    // Trigger export AFTER transaction success
    ctx.waitUntil(runFullExport(env));
...
let rawIndexState = await env.EXPORT_STATE.get("search_index_state");
let searchIndex;

try {
  searchIndex = rawIndexState ? JSON.parse(rawIndexState) : [];
} catch {
  searchIndex = [];
}
    while (true) {
...
if (isFinished) {
  await uploadIndex(env, searchIndex);
...
async function uploadIndex(env, indexData) {
  ...
  // Convert array to NDJSON (1 object per line)
  ...
  await fetch(
    `https://api.github.com/repos/${env.GITHUB_REPO}/contents/${path}`,
    {
      ...
      body: JSON.stringify({
        message: `Auto export index.json (NDJSON format)`,
        content: base64Content,
        ...(sha && { sha })
      })
    }
