...
if (method === "DELETE" && url.pathname === `/${env.ADMIN_DELETE}/anime/${url.pathname.split("/").pop()}`) {
  const id = url.pathname.split("/").pop();
  return await deleteAnime(db, id);
}

if (method === "POST" && url.pathname === `/${env.ADMIN_CREATE}/anime`) {
  return await createAnime(request, db);
}

if (method === "GET" && url.pathname === `/${env.ADMIN_GET}/anime-ids`) {
  return await getAnimeIds(db);
}

if (method === "GET" && url.pathname.startsWith(`/${env.ADMIN_GET}/anime/`)) {
  const id = url.pathname.split("/").pop();
  return await getAnime(db, id);
}

if (method === "PUT" && url.pathname.startsWith(`/${env.ADMIN_UPDATE}/anime/`)) {
  const id = url.pathname.split("/").pop();
...
function json(data, status = 200) {
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      "Access-Control-Allow-Origin": "https://your-panel-domain.com",
